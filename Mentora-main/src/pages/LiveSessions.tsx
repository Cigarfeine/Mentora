      ...
      <div className="lg:col-span-2 space-y-10">
        <section>
          <div className="flex items-center justify-between mb-6">
            <BlurText text="Upcoming Live Sessions" className="text-3xl font-bold text-slate-900" delay={0.1} />
            <Link to="/live-sessions"><button className="text-sm font-medium text-[#0f4c81] hover:text-[#0c3d68]">See All</button></Link>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {LIVE_SESSIONS.map((session, i) => (
              <Link to={`/live-sessions/${session.id}`} key={session.id} className="block">
                <motion.div 
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: i * 0.1 }}
                  whileHover={{ y: -4 }}
                  className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col hover:shadow-lg transition-all cursor-pointer h-full"
                >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="bg-slate-50 rounded-xl p-5 mb-6 border border-slate-100">
              <div className="flex flex-col xl:flex-row items-center gap-4 mb-4">
                <Link to="/subjects#syllabus" className="w-full xl:flex-1">
                  <button className="w-full bg-white border border-slate-200 py-2.5 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 shadow-sm">Notes & Syllabus</button>
                </Link>
                <button className="w-full xl:flex-1 bg-white border border-slate-200 py-2.5 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 shadow-sm">Quiz</button>
              </div>
            ...
            <div className="space-y-4">
              {MENTORS.map((mentor, i) => (
                <Link to="/community" key={mentor.id} className="block">
                  <motion.div 
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.1 }}
                    className="shadow-sm border border-transparent hover:border-slate-200 transition-colors cursor-pointer rounded-xl overflow-hidden"
                  >
